...
class Zh_cjdiancModuleWxapp extends WeModuleWxapp {
  ...
  //分类下菜品
	public function doPageDishes(){
		global $_W, $_GPC;
		$res=pdo_getall('cjdc_goods',array('type_id'=>$_GPC['type_id'],'is_show'=>1,'type !='=>$_GPC['type']));
		echo json_encode($res);
	}
...
public function doPageDishesList() {
    global $_W, $_GPC;
  //  $type = pdo_getall('cjdc_type', array('uniacid' => $_W['uniacid'], 'store_id' => $_GPC['store_id'], 'is_open' => 1), array(), '', 'order_by ASC');
    $sql=" select * from".tablename('cjdc_type')." where  uniacid={$_W['uniacid']} and store_id={$_GPC['store_id']} and is_open=1 and id in(select type_id from".tablename('cjdc_goods')." where uniacid={$_W['uniacid']} and is_show=1 and type !={$_GPC['type']} and store_id={$_GPC['store_id']})";
...
//热销
	public function doPageHot(){
		global $_W, $_GPC;
		$res=pdo_getall('cjdc_goods',array('store_id'=>$_GPC['store_id'],'is_hot'=>1,'is_show'=>1,'type !='=>$_GPC['type']));
		echo json_encode($res);
	} 
	//商家列表
...
//改变订单状态
    public function doPagePayOrder(){
        ...
        if($order['type']==1){//外卖
	        ...
	        }elseif($store['is_jd']==1&&$order['order_type']==2){
	        	$data['state']=3;	
	        	$data['pay_time']=date('Y-m-d H:i:s'); 
	        	$data['jd_time']=date('Y-m-d H:i:s');	
	        }else{
    ...
    public function doPageOrderInfo(){
      ...
      $res5=pdo_fetch($sql);
       $sql2="select  a.*,b.name as type_name from " . tablename("cjdc_table")  . " a"  . " left join " . tablename("cjdc_table_type") . " b on b.id=a.type_id where b.id=".$res['table_id'];
      $res6=pdo_fetch($sql2);
...
public function doPageSaveNumber(){
	global $_W, $_GPC;
	//判断是否可以取号
	$mynumber=pdo_get('cjdc_number',array('user_id'=>$_GPC['user_id']));
if($mynumber['state']==3 or !$mynumber ){
	...
	$data['state']=1;
 	$data['time']=date('Y-m-d H:i:s');
	$data['uniacid']=$_W['uniacid'];
	$sql=" select count(id) as count from".tablename('cjdc_number')." where  store_id={$_GPC['store_id']} and time LIKE '%{$time}%' ";
	$rst=pdo_fetch($sql);
	if($rst['count']>0){
		$num=$rst['count']+1;
		$data['code']="A000".$num;
...
//我的号
public function doPageMyNumber(){
	global $_W, $_GPC;
	$time=date("Y-m-d",time());
	$sql=" select id,code,num as count from".tablename('cjdc_number')." where  store_id={$_GPC['store_id']} and time LIKE '%{$time}%' ";
	$list=pdo_fetch($sql);
...
public function doPageSaveRzsq(){
	...
	$data['state'] = 1;  
	if(strlen($_GPC['logo'])<25){
		$data['logo']=$_W['attachurl'].$_GPC['logo'];
	}else{
		$data['logo']=$_GPC['logo'];
	}
	$data['yyzz']=$_GPC['yyzz'];
	// if(strlen($_GPC['yyzz'])<25){
	// 		$data['yyzz']=$_W['attachurl'].$_GPC['yyzz'];
	// 	 }else{
	// 	 	$data['yyzz']=$_GPC['yyzz'];
	// 	 }
	if(strlen($_GPC['fm_img'])<25){
			$data['fm_img']=$_W['attachurl'].$_GPC['fm_img'];
		 }else{
		 	$data['fm_img']=$_GPC['fm_img'];
		 }	 
	if(strlen($_GPC['zm_img'])<25){
			$data['zm_img']=$_W['attachurl'].$_GPC['zm_img'];
		 }else{
...
//短信验证码
	...
	public function doPageCheckRz(){
		global $_W, $_GPC;
		$res=pdo_get('cjdc_store',array('uniacid'=>$_W['uniacid'],'sq_id'=>$_GPC['user_id'],'zf_state'=>2));
		echo json_encode($res);
	}
...
public function doPageStoreWmOrder(){
	global $_GPC, $_W;
	if($_GPC['zt']==1){
		$where=" WHERE store_id=".$_GPC['store_id']." and state in (".$_GPC['state'].") and type=1 and order_type=2";
	}elseif($_GPC['zt']==2){
		$where=" WHERE store_id=".$_GPC['store_id']." and state in (".$_GPC['state'].") and type=1  and order_type=1";
	}else{
		$where=" WHERE store_id=".$_GPC['store_id']." and state in (".$_GPC['state'].") and type=1";
	}	
	$pageindex = max(1, intval($_GPC['page']));
	$pagesize=$_GPC['pagesize'];
	$sql="select  *  from " . tablename("cjdc_order") ." ".$where." order by id DESC";
	$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$list = pdo_fetchall($select_sql);
	$good=pdo_getall('cjdc_order_goods',array('uniacid'=>$_W['uniacid']));
	$data2=array();
...
//商品分类
   ...
   public function doPagecjpt(){
   	...
   	$order_id=$_GPC['order_id'];
    	...
    	$order=pdo_get('cjdc_order',array('id'=>$order_id));
    	$store=pdo_get('cjdc_store',array('id'=>$order['store_id']),array('name','address','tel','coordinates'));
...
public function doPageAddStoreDishes(){
	...
	if($_GPC['logo']!=$res['logo']){
		$data['logo']=$_W['attachurl'].$_GPC['logo'];
	}
	// if($_GPC['label_id']){
	// 	$data['label_id']=$_GPC['label_id'];
	// }
	if($_GPC['money']){
		$data['money']=$_GPC['money'];
	}
...
public function doPageIsPay(){
	global $_W, $_GPC;
	$res=pdo_get('cjdc_qgorder',array('user_id'=>$_GPC['user_id'],'good_id'=>$_GPC['good_id'],'state'=>array(2,3)));
	if($res){
		echo '1';
	}else{
		echo '2';
...
public function doPageMyGroupOrder(){
	...
	$pageindex = max(1, intval($_GPC['page']));
	$pagesize=empty($_GPC['pagesize'])?10:$_GPC['pagesize'];
	$sql="select a.*,b.state as g_state from " .tablename("cjdc_grouporder"). " a left join ".tablename('cjdc_group') ." b on a.group_id=b.id" .$where." order by a.id DESC";
	$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$res=pdo_fetchall($select_sql);
	echo json_encode($res);
